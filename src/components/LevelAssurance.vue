<template>
  <img src="/assets/arrowUp.png" alt="Arrow" style="margin-top: -1rem; height: 5rem" />
  <div class="row">
    <img src="/assets/arrowLeft.png" class="g-0" alt="Left Arrow" style="margin-left:-2rem; width:5rem" />
    <div id="levels" class="col g-0">Levels of Assurance</div>
    <img src="/assets/arrowRight.png" class="g-0" alt="Right Arrow" style="margin-right:-2rem;width: 5rem" />
  </div>  
  <img src="/assets/arrowDown.png" alt="Arrow" style="margin-bottom: -1rem; height: 5rem" />
</template>

<style scoped>
div#levels{
  border-radius: 50%;
  background-color:#00B0F0;
  color: black;
  padding: 1rem;
  line-height: 1rem;
}
</style>